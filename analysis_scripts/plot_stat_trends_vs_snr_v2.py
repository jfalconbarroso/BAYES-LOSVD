...
from   compute_stats_v2      import compute_stats
#==============================================================================
...
#    dir = "../results_denso/"
    ...
    filelist = glob.glob(dir+"testcases_*/testcases*results.hdf5")
    ...
    colors  = ['red','blue','green','coral','magenta','brown','yellow']
    ...
    for j in range(ncases):
        for k in range(nftype):
            lo, hi, med = np.zeros((3,nftype,nsnr))*np.nan, np.zeros((3,nftype,nsnr))*np.nan, np.zeros((3,nftype,nsnr))*np.nan
            for o in range(nsnr):
                idx = (tab['case'] == cases[j]) & (tab['Fit type'] == ftype[k]) & (tab['S/N'] == float(snr[o]))
            ...
            ax[0,j].fill_between(np.array(snr,dtype=float), lo[0,k,:], hi[0,k,:], alpha=0.15, color=colors[k])
            ax[0,j].plot(np.array(snr,dtype=float), med[0,k,:], '.-', color=colors[k], label=ftype[k])
            ax[1,j].fill_between(np.array(snr,dtype=float), lo[1,k,:], hi[1,k,:], alpha=0.15, color=colors[k])
            ax[1,j].plot(np.array(snr,dtype=float), med[1,k,:], '.-', color=colors[k], label=ftype[k])
            ax[2,j].fill_between(np.array(snr,dtype=float), lo[2,k,:], hi[2,k,:], alpha=0.15, color=colors[k])
            ax[2,j].plot(np.array(snr,dtype=float), med[2,k,:], '.-', color=colors[k], label=ftype[k])
