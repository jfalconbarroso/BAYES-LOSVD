...
def load_templates(struct,data_struct):
   ...
   # NOTE: these loops already cut the spectra to the Lmin,Lmax limits
   ...
   if npca > 0:
      # IF npca > 0, Normalizing the mean template to 1.0 and adjusting the 
      # other templates so that the mean is around 0.0
      mean_temp /= np.mean(mean_temp)
      for i in range(ntemplates):
          templates[:,i] -= np.mean(templates[:,i])
   else:       
      # IF npca = 0, setting the mean template to 0.0 and adjusting the 
      # other templates so that the mean is around 1.0
