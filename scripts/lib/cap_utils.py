...
def display_pixels(x, y, val, pixelsize=None, vmin=None, vmax=None,
                   angle=None, colorbar=True, label=None, nticks=7, cmap=None, **kwargs):
    """
    ...
    ax.minorticks_on()
    ax.tick_params(length=5, width=1, which='major')
    ax.tick_params(length=2.5, width=1, which='minor')
...
def display_bins(x, y, binNum, velBin, cmap=None, pixelsize=1.0, vmin=0, vmax=1, colorbar=False):
    ...
    img = display_pixels(x, y, velBin[binNum], cmap=cmap, pixelsize=pixelsize, vmin=vmin, vmax=vmax, colorbar=colorbar)
